...
contract deployAll is Script {
    ...
    address constant COLLATERAL_ADDRESS =
        0xeb762Ed11a09E4A394C9c8101f8aeeaf5382ED74; // eth sepolia
    ...
    address constant DELEGATOR_FACTORY =
        0x890CA3f95E0f40a79885B7400926544B2214B03f; // Replace with deployed DelegatorFactory address
    address constant SLASHER_FACTORY =
        0xbf34bf75bb779c383267736c53a4ae86ac7bB299; // Replace with deployed SlasherFactory address
    ...
    function run() external {
        ...
        iBTC_Treasury treasury = new iBTC_Treasury(
            COLLATERAL_ADDRESS,
            MAX_WITHDRAW_AMOUNT,
            MIN_WITHDRAW_AMOUNT
        );
        ...
        iBTC_Burner burner = new iBTC_Burner(
            COLLATERAL_ADDRESS,
            address(treasury)
        );
            ...
            slasherParams = abi.encode(
                ISlasher.InitParams({
                    baseParams: IBaseSlasher.BaseParams({
                        isBurnerHook: address(burner) != address(0)
                    })
                })
            );
